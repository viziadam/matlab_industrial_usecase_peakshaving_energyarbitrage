...
%   run_ac_dc_diagnostic_plots_from_saved_results("combined", [9 12 15])
    ...
    dcMetricsDetailed = load_canonical_candidate_metrics( ...
        cfgBase, ...
        objectiveMode, ...
        "dc", ...
        candidateList, ...
    ...
    acMetricsDetailed = load_canonical_candidate_metrics( ...
        cfgBase, ...
        objectiveMode, ...
        "ac", ...
        candidateList, ...
    ...
    dcMetricsImportant = load_canonical_candidate_metrics( ...
        cfgBase, ...
        objectiveMode, ...
        "dc", ...
        candidateList, ...
    ...
    acMetricsImportant = load_canonical_candidate_metrics( ...
        cfgBase, ...
        objectiveMode, ...
        "ac", ...
        candidateList, ...
    ...
    Tdc = local_as_table(dcMetricsDetailed, "dcMetricsDetailed");
    Tac = local_as_table(acMetricsDetailed, "acMetricsDetailed");

    [Tdc, Tac, candidateListUsed] = local_align_dc_ac_tables(Tdc, Tac, candidateList);

    local_validate_required_diagnostic_columns(Tdc, "DC");
    local_validate_required_diagnostic_columns(Tac, "AC");
    ...
    out.dc.importantMetrics = dcMetricsImportant;
    out.dc.detailedMetrics = Tdc;

    out.ac.importantMetrics = acMetricsImportant;
    out.ac.detailedMetrics = Tac;
...
function local_validate_required_diagnostic_columns(T, label)
    ...
    if ~isempty(missingColumns)
        fprintf('\nMissing required diagnostic columns in %s table:\n', label);

        for i = 1:numel(missingColumns)
            fprintf('  - %s\n', missingColumns{i});
        end

        error(['The saved detailed candidate metrics are incomplete. ', ...
               'Do not plot from legacy/aggregated data. Fix cfg.output metrics, rerun the selected candidates, ', ...
               'and regenerate the canonical metric cache.']);
    end
end