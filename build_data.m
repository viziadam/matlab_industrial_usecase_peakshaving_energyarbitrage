...
%     % =====================================================================
%     requiredTopFields = {'pvA', 'pvB', 'pv', 'loadScale', 'targetStepMin', 'aggregationMethod'};
% 
...
%   - Ha adott load naphoz nincs PV_A/PV_B a hozzárendelt PV évben,
%     akkor az a nap kimarad.
%   - A kimeneti szimulációs időlépést cfg.targetStepMin adja meg.
%
% Kimenet:
%   data.days(d).date
%   data.days(d).P_load_kW
...
%   data.days(d).dt_h
    ...
    if ~isnumeric(cfg.targetStepMin) || ~isscalar(cfg.targetStepMin) || ...
       ~isfinite(cfg.targetStepMin) || cfg.targetStepMin <= 0
        error('cfg.targetStepMin must be a positive finite scalar.');
    end

    aggregationMethod = lower(string(cfg.aggregationMethod));
    ...
    if aggregationMethod ~= "mean"
        error('Jelenleg csak cfg.aggregationMethod = "mean" támogatott.');
    end

    target_dt_h = cfg.targetStepMin / 60;

    fprintf('Load scale factor: %.4f\n', cfg.loadScale);
    fprintf('Target simulation timestep: %.3f min\n', cfg.targetStepMin);
    fprintf('Aggregation method: %s\n', aggregationMethod);

    % =====================================================================
    % 1/b) Feldolgozott build_data RAM-cache
    % =====================================================================
    persistent BUILD_DATA_CACHE

    if isempty(BUILD_DATA_CACHE)
        BUILD_DATA_CACHE = containers.Map('KeyType', 'char', 'ValueType', 'any');
    end

    cacheKey = sprintf( ...
        'ls_%g_step_%g_agg_%s_pvA_X%s_Z%s_P%s_pvB_X%s_Z%s_P%s', ...
        cfg.loadScale, ...
        cfg.targetStepMin, ...
        char(aggregationMethod), ...
        mat2str(cfg.pvA.tiltX), ...
        mat2str(cfg.pvA.tiltZ), ...
        mat2str(cfg.pvA.P_dc_kWp), ...
        mat2str(cfg.pvB.tiltX), ...
        mat2str(cfg.pvB.tiltZ), ...
        mat2str(cfg.pvB.P_dc_kWp));

    cacheKey = regexprep(cacheKey, '[^\w\d_]', '_');

    if BUILD_DATA_CACHE.isKey(cacheKey)
        fprintf('Industrial synchronized data loaded from build_data RAM cache: %s\n', cacheKey);
        data = BUILD_DATA_CACHE(cacheKey);
        return;
    end
    ...
    for i = 1:numel(Load)
        ...
        % -----------------------------------------------------------------
        % PV időlépés illesztése a Load/Price időlépéshez
        % -----------------------------------------------------------------
        if abs(PA.dt_h - dt_load_h) > 1e-12
            P_pv_A_dc_kW = local_resample_power_to_target_dt( ...
                P_pv_A_dc_kW, PA.dt_h, dt_load_h, aggregationMethod);
        end

        if abs(PB.dt_h - dt_load_h) > 1e-12
            P_pv_B_dc_kW = local_resample_power_to_target_dt( ...
                P_pv_B_dc_kW, PB.dt_h, dt_load_h, aggregationMethod);
        end

        % -----------------------------------------------------------------
        % Teljes napi idősor átállítása a szimulációs cél-időlépésre
        % -----------------------------------------------------------------
        if abs(target_dt_h - dt_load_h) > 1e-12

            P_load_kW = local_resample_power_to_target_dt( ...
                P_load_kW, dt_load_h, target_dt_h, aggregationMethod);

            P_pv_A_dc_kW = local_resample_power_to_target_dt( ...
                P_pv_A_dc_kW, dt_load_h, target_dt_h, aggregationMethod);

            P_pv_B_dc_kW = local_resample_power_to_target_dt( ...
                P_pv_B_dc_kW, dt_load_h, target_dt_h, aggregationMethod);

            buy_huf = local_resample_power_to_target_dt( ...
                buy_huf, dt_load_h, target_dt_h, aggregationMethod);

            sell_huf = local_resample_power_to_target_dt( ...
                sell_huf, dt_load_h, target_dt_h, aggregationMethod);

            dt_load_h = target_dt_h;
        end
    ...
    % =====================================================================
    % Konzisztencia ellenőrzés a kész data.days struktúrára
    % =====================================================================
    ref_dt_h = data.days(1).dt_h;
    ref_nT = numel(data.days(1).P_load_kW);

    for d = 1:numel(data.days)

        if abs(data.days(d).dt_h - ref_dt_h) > 1e-12
            error('Eltérő dt_h a kész data.days struktúrában. Nap index: %d', d);
        end

        if numel(data.days(d).P_load_kW) ~= ref_nT || ...
           numel(data.days(d).P_pv_A_dc_kW) ~= ref_nT || ...
           numel(data.days(d).P_pv_B_dc_kW) ~= ref_nT || ...
           numel(data.days(d).Price.buy_huf) ~= ref_nT || ...
           numel(data.days(d).Price.sell_huf) ~= ref_nT

            error('Eltérő napi vektorhossz a kész data.days struktúrában. Nap index: %d', d);
        end
    end
    ...
    data.info.nDays = numel(data.days);
    data.info.dt_h = data.days(1).dt_h;
    data.info.targetStepMin = cfg.targetStepMin;
    data.info.aggregationMethod = aggregationMethod;
    data.info.nT = numel(data.days(1).P_load_kW);
    ...
    fprintf('Steps per day: %d\n', data.info.nT);
    fprintf('dt_h: %.6f h\n', data.info.dt_h);
    fprintf('Target step: %.3f min\n', data.info.targetStepMin);
    fprintf('Aggregation method: %s\n', data.info.aggregationMethod);
    fprintf('Skipped days due to missing PV month-day: %d\n', skippedMissingPV);
    fprintf('Skipped days due to vector mismatch: %d\n', skippedVectorMismatch);
    fprintf('PV_A: %.3f kWp\n', data.info.P_pv_A_dc_kWp);
    fprintf('PV_B: %.3f kWp\n', data.info.P_pv_B_dc_kWp);
    ...
    BUILD_DATA_CACHE(cacheKey) = data;
end
...
function y = local_resample_power_to_target_dt(x, dt_in_h, dt_out_h, aggregationMethod)

    x = x(:).';

    if nargin < 4 || isempty(aggregationMethod)
        aggregationMethod = "mean";
    end

    aggregationMethod = lower(string(aggregationMethod));
    ...
    if aggregationMethod ~= "mean"
        error('local_resample_power_to_target_dt: only mean aggregation is supported.');
    end

    if dt_in_h <= 0 || dt_out_h <= 0
        error('local_resample_power_to_target_dt: dt values must be positive.');
    end
    ...
    if dt_out_h > dt_in_h
        ...
        % Finomabb adatból durvább adat:
        % teljesítmény-idősorok esetén átlagolás.
        %
        % Példa:
        %   5 perc -> 15 perc
        %   factor = 3
        %
        % Energia-megmaradás:
        %   mean(P_5min) * 15 perc = sum(P_5min * 5 perc)
        ...
        % Durvább adatból finomabb adat:
        % zero-order hold jellegű ismétlés.
        %
        % Ez nem hoz létre új mérési információt, csak a meglévő átlagos
        % teljesítményszintet tartja konstansként a kisebb lépésközökön.
